  ...
  for (var i = 0; i < player.shurikensInAir.length; i++) {
    ...
    if (player.shurikensInAir[i][0] == "left") {
      while (newX > (player.shurikensInAir[i][1] - player.shurikenVelocity)) {
        ...
        if (isObjectCollidingWithTilemapLeft(newX, player.shurikensInAir[i][2] + Game.shurikenHitboxToTilemapDecreaser, Game.shurikenWidth - Game.shurikenHitboxToTilemapDecreaser * 2, Game.shurikenHeight - Game.shurikenHitboxToTilemapDecreaser * 2) || player.shurikensInAir[i][1] <= 0) {
    ...
    // Right.
      ...
      while (newX < (player.shurikensInAir[i][1] + player.shurikenVelocity)) {
        ...
        if (isObjectCollidingWithTilemapRight(newX, player.shurikensInAir[i][2] + Game.shurikenHitboxToTilemapDecreaser, Game.shurikenWidth - Game.shurikenHitboxToTilemapDecreaser * 2, Game.shurikenHeight - Game.shurikenHitboxToTilemapDecreaser * 2) || player.shurikensInAir[i][1] >= Game.width - Game.shurikenWidth) {
          killThisShuriken = true;
        }
      }
    }
